...
def evaluate_gsc(model, enroll_data, test_data, num_enrollment=10):
    ...
    for target in enroll_data.keys():
        ...
        # 計算中心點
        centroid = torch.stack(enroll_embeddings, dim=0).mean(dim=0)  # [embedding_dim]
        ...
        for test_feature in test_wavs:
            ...
            positive_similarities.append(sim.item())  # 只需要取標量值

        # 測試數據 (負樣本)
        negative_similarities = []
        for other_target, other_test_wavs in test_data.items():
        ...
        # 合併正負樣本並生成標籤
        positive_similarities = np.array(positive_similarities)  # [num_positive_samples]
        negative_similarities = np.array(negative_similarities)  # [num_negative_samples]
        similarities = np.concatenate([positive_similarities, negative_similarities])  # [num_samples]
...
def main(args):
    ...
    # 如果指定了關鍵詞，過濾 enroll 資料
    if args.keywords:
        keywords_to_evaluate = args.keywords.split(',')
        enroll_data = {key: value for key, value in enroll_data.items() if key in keywords_to_evaluate}
        print(f"Evaluating only these keywords: {', '.join(enroll_data.keys())}")
    else:
        print(f"Evaluating all keywords.")
